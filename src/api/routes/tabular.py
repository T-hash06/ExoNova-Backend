...
from ...ml.tabular_classifier import TabularClassifier
...
async def predict_tabular_endpoint(
    ...
    try:
        # Enviar solo los campos no nulos al predictor
        input_data = request.model_dump(exclude_none=True)

        predicted_value, confidence, attribute_weights = predictor.predict(input_data)
        ...
        logger.info(
            f"Tabular prediction response {request_id}: predictedValue={predicted_value:.4f}, confidence={confidence:.4f}"
        )

        return BaseResponse[TabularPredictionResponse](
            data=response_data, timestamp=timestamp, requestId=request_id
        )
